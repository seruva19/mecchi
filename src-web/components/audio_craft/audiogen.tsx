...
const MecchiAudioGenNodeInfo = {
  type: 'audiogen',
  group: 'generate',
  view: MecchiAudioGenNode,
  data: {
  ...
  transform: function (inputs: MecchiKV, state: MecchiKV): Promise<MecchiKV> {
    return new Promise(async resolve => {
      ...
      const result: any = await ky.post('/mecchi/audiogen', {
        json: payload,
        timeout: false
      }).json();
...
export default MecchiAudioGenNodeInfo;

const selector = (id: string) => (store: any) => ({
  setParams: (key: string, value: any) => store.updateNode(id, { [key]: value }),
});

export function MecchiAudioGenNode({ id, data }: { id: string, data: MecchiKV }) {
  const { setParams } = useMecchiNodeStore(selector(id));

  return <MecchiNode title="AudioGen" id={id}>
    <Handle id="musicgen-a" className={`${tw`w-2 h-2`}`} type="target" style={{ background: 'orange' }} position={Position.Bottom}>
    </Handle>
